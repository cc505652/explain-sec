// Console-Friendly Auto-Assignment Diagnostic
// Copy and paste this entire script into the browser console

(async function diagnoseAutoAssignment() {
  try {
    console.log("🔍 Diagnosing Auto-Assignment System...");
    
    // Get Firebase instances from window (they should be available globally)
    const { getDocs, collection, doc, updateDoc, query, where } = window.firebase.firestore;
    const db = window.firebaseDb; // This should be available from your imports
    ...
    if (analysts.length === 0) {
      ...
      if (!hasSkills) {
        ...
        console.log(`   💡 Run: addBasicSkillsToAnalysts()`);
      }
    }
    ...
    console.log("💡 Make sure you're on a dashboard page and Firebase is loaded");
    return null;
  }
})();

// Quick fix function to add basic skills to all analysts
(async function addBasicSkillsToAnalysts() {
  try {
    console.log("🔧 Adding basic skills to all analysts...");
    
    const { getDocs, collection, doc, updateDoc } = window.firebase.firestore;
    ...
    console.log(`🔄 Now run diagnoseAutoAssignment() again to check!`);
...
console.log("\n🚀 Quick usage:");
console.log("  diagnoseAutoAssignment();");
console.log("  addBasicSkillsToAnalysts();");
